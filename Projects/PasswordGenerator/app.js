...
function generatePassword(lower, upper, number, symbol, length){
    ...
    // True & False values can be added together (True = 1 and False = 0)
    // NOTE: The value set to types count will be used when building the password
    const typesCount = lower + upper + number + symbol;
    console.log(typesCount);

    // If the user has not selected any of the options then display alert and return an empty string from the function so the password displayed will be an empty string
    if (typesCount === 0){
        alert(`Please select at least ONE option!!`);
        // The RETURN keyword stops/ends the execution of a function (AKA Does not run any of the on the lines that follow the return in the function)
        return ``;
    }

    // Creating an array of arays. The first item in each nested array holds the value of a string that will be used to access a function in the randomFunctions object. Also, the second item in each nested array is one of the values passed into this generatePassword function.
    let typesArr = [
        [`lower`, lower],
        [`upper`, upper],
        [`number`, number],
    ...
    for (i = 0; i < length; i += typesCount){
        // One of the items in the updated/filtured version of the typesArr will be the value/argument passed in for the type parameter each time the anonymous arrow function is run/executed
    ...
    // ! 4. ADD GENERATED PASSWORD TO FINAL PASSWORD VARIABLE AND RETURN IT FROM THE FUNCTION ! \\
...
generateEl.addEventListener(`click`, () => {
    // Checking if the following options/checkboxes are selected/checked and setting the true/false values to the respective variables
